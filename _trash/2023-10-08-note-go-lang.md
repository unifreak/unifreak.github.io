...
exercise your packages using concurrency**.
...
reflection error is reported during execution as a panic, possibly long after
the program was written or even long after it has started running.

best way to avoid this fragility is to ensure that the use of reflection is
fully encapsulated within your package and, if possible, If this is not
...
3. reflection-based functions may be one or two orders of magnitude slower than
code specialized for a particular type. In a typical program, the majority of
functions are not relevant to the overall performance, so its fine to use
...
The reflect.TypeOf function accepts any interface{} and returns its dynamic type
as a reflect.Type:
...
We can ask a reflect.Value whether it is addressable through its CanAddr
method:
...
it will be quietly truncated to fit. But tread care- fully: calling SetInt on a
...
(i) call returns an instance of reflect.Method, a struct type that describes
...
the *unsafe* package lets us step outside the usual rules, and how to use
the *cgo* tool to create Go bindings for C libraries and operating system
calls.

it is actually implemented by the compiler. is used extensively within low-level
packages like runtime, os, syscall, and net that interact with the operating
...
Offsetof(x.f) computes the offset of filed relative to the start of its
enclosing struct x.
...
any variable. Of course, we can't indirect through an unsafe.Pointer using `*p`
because we don't know what type that expression should have. Like ordinary
