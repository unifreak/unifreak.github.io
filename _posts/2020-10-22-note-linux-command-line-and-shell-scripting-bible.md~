...
1. 直接在终端使用多个命令, 同一行的多个命令用 `;` 分开. 每行不能超过最大命令行字符数 *255*.
   这种方式缺点在于脚本不能重用.
2. 创建并运行 shell 脚本文件. 使用这种方法要注意:
  - 文件第一行指定要使用的 shell, 如 `#!/bin/bash`
  - 必须让 shell 能找到脚本. 可以将 shell 脚本文件所处目录添加到 PATH 环境变量中,
    也可以在提示符中用绝对或相对文件路径来引用脚本文件
...
`''` 或 `""` 可以划分一个字符串. `""` 中的变量会被解析, 如 `echo "var is $var"`. 要输出
`$`, 需要转义 `\$`.
...
是可以嵌套的, 因为一旦在 $(...) 子 shell 之内, 双引号从头解析. 比如下面这样是没有问题的:
...
使用**内联输入重定向**, 可以无需使用文件, 直接在命令行输入, marker 标识输入的起始: `cmd << marker`,
如:
...
浮点运算精度由内建变量 `scale` 控制, 其默认值为 0. 必须将这个值**设置为期望的计算结果中保留的小数位数**.
...
脚本默认以**最后一个命令的退出状态码退出**. 但可以使用 `exit code` 自己指定退出状态码, 指定的
状态码最大为 255, 大于 255 的会自动模 255.
...
它**根据命令返回的退出状态码**决定是否执行, 退出状态码为 0 则执行. 如果你想以类似其他变成语言中
的 if-then 语句的方式工作, 可以使用 test 命令或者使用测试表达式 `[ ]`.

test 命令格式为: `if test condition`. 如果不写 condition 部分, test 命令会以非零退出状态
码退出. 如果加入 condition, test 命令则会测试该条件.

测试表达式 `[ condition ]` 实际上是 `test` 的别名. 注意 condition 两边的**空格是必须的**.
...
注意, 必须**正确转义 > 和 <**, 否则 shell 会将其解释为重定向. 另外, 大写字母被认为大于小写字
母, 这和 `sort` 相反.
...
test 命令和 `[]` 测试表达式用标准的数学比较符号来表示字符串比较, 而用文本代码表示数值比较,
**不要记反了**. 如果对数值使用了数学运算符号, shell 会将它们当成字符串值进行比较.
...
`[[ ]]` 用于字符串比较 (不是所有 shell 都支持). 除了支持 test 的标准字符串比较, `[[ ]]` 还
提供**模式匹配**功能.
...
函数的默认退出状态码是其最后一条语句的退出状态码, 所以一般不要用函数的默认退出状态码, 因为你不知
道其他命令是否执行成功.
...
如果你试图将数组变量作为函数参数, 函数**只会取到数组变量的第一个值**. 要解决这个问题, 必须先打
散数组, 传给函数, 然后在函数内部重组函数.
...
如果将库文件的函数添加到 `.bashrc` 中, 则启动 shell 时, 所定义的函数便都可使用了. 更好的是,
...
shift n 会将每个参数变量向左移动 n(默认 1) 个位置. 可以使用 shift 遍历参数, 尤其在不知道参
数有多少个的时候.
...
**选项**是跟在单破折线后面的单个字母, 它能改变命令的行为. 选项和参数并无特殊之处, 也可以像处理
参数一样处理选项.
...
* 使用 getopt 处理选项.
    ...
    其中 opstring 是关键, 它定义了命令行有效的选项字母, 还**使用冒号**定义了哪些选项字母需要
    参数值.
    ...
    getopt 不擅长处理带空格和引号的参数值, 如对于 `-d "foo bar"`, getopt 会将空格作为分隔
    符, 当做两个参数.

* 使用 getopts (注意多了 `s`).

    与 getopt 不同, 前者将命令行上选项和参数处理后只生成一个输出. 而 getopts 则是, 每次调用
    它时, 它一次只处理命令行上检测到的一个参数, 并且如果选项需要参数值, OPTARG 环境变量会保存
    这个值, OPTIND 环境变量则保存了参数列表中 getopts 正在处理的参数位置. 处理完所有参数后,
    它会退出并返回一个大于 0 的退出状态码. 这让它非常**适合用于解析命令行所有参数的循环中**.
...
Linux 用**文件描述符(FD)**标识每个文件对象. FD 是一个非负整数, 每个进程最多有**9**个FD, 有
三个预定义的 FD:
...
* 如果将错误和数据重定向到同一个文件, 可以使用特殊的重定向符号 `&> errout`. 注意, shell 自
  动赋予了错误消息更高的优先级, 所以**错误会优先于数据集中输出在前面**.
...
使用 mktemp 命令会在本地目录创建一个临时文件, 只要指定一个文件名模板就行. 如运行 `mktemp tmp.XXXXXX`,
系统会自动创建模板为 `tmp.XXXXXX` 且文件名唯一的文件 (如 `tmp.UfIi13`).
...
为了将输出同时发送到显示器和日志文件, 可以使用 tee 命令. `tee file` 将从 STDIN 过来的数据同
时发往 STDOUT 和指定的 file. 选项 -a 则会让 tee 追加到文件, 而非覆盖.
